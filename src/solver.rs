...
mod tests {
    ...
    fn test_five_twos_equals_1024() {
        // Test that "22222" can produce 1024 via (2 ^ (2 + (2 × (2 ^ 2))))
        // Expected: (2 ^ (2 + (2 × (2 ^ 2)))) = 1024
        let solver = ExpressionSolver::new();
        let result = solver.find_expression("22222", 1024.0);

        assert!(
            result.is_some(),
            "Should find an expression that equals 1024 using all five 2's"
        );
        ...
        if let Some(expr) = result {
            ...
            assert!(
                (value - 1024.0).abs() < 1e-9,
                "Expression should evaluate to 1024, got {}",
                value
            );

            let expr_str = format!("{}", expr);
            println!("Found expression for 22222->1024: {}", expr_str);

            // Verify it contains five 2's
            let two_count = expr_str.matches('2').count();
            assert_eq!(
                two_count, 5,
                "Expression should contain exactly 5 twos, found {} in: {}",
                two_count, expr_str
            );
        }
    }
